...
export default function People() {
  ...
  const getUserProfiles = () => {
    Axios.get("http://localhost:8082/get_user_profiles")
      .then(res => {
        console.log("get_user_profiles api", res)
        if (res.status === 200) {
          const { data } = res
          data.map(profile => {
            //  if  profile is already present in table then dont add again
            if (rows.findIndex(obj => obj.id === profile.fr_id) === -1) {
              dispatch(
                add({
                  name: profile.fr_title,
                  id: profile.fr_id,
                  amount: profile.fr_target,
                  img: "https://dummy.com",
                })
              )
            }
      ...
      .catch(err => console.log("error get_user_profiles api", err))
  }
